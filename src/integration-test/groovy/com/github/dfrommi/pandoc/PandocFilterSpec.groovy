...
class PandocFilterSpec extends Specification {
  ...
  def "toJSONFilter transformation #action elements"() {
    ...
    [action, mdInput, mdExpectedResult, transformation] << [
      ...
      ["adds", "Hello world", "Good morning world", {
        if (it in Str && it.text == "Hello") [new Str("Good"), new Space(), new Str("morning")]
      }],
      ["replaces multiple", "# Some header\n\nSome text", "Header1\n\nHeader2\n\nSome text", {
        if (it in Header) [new Para(new Str("Header1")), new Para(new Str("Header2"))]
      }],
